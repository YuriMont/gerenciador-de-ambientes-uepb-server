 ...
 *
 * <p>
 * Executado automaticamente apenas quando o perfil ativo é {@code test} (definido pela variável
 * {@code APP_PROFILE}). Cria perfis de exemplo em todas as hierarquias ({@code OWNER},
 * {@code ADMIN} e {@code USER}), ambientes físicos da UEPB e reservas em todos os status, incluindo
 * para a data atual — o que alimenta o painel da tela de início e a fila de aprovação.
 * </p>
 *
 * <p>
 * O seeder é <strong>idempotente</strong>: usuários são verificados por e-mail e ambientes por
 * nome, então executar a aplicação novamente apenas completa o que ainda não existe.
 * </p>
 *
 * <p>
 * <strong>Credenciais geradas:</strong> todos os usuários usam a senha {@code senha@123},
...
public class TestDataSeeder implements ApplicationListener<ContextRefreshedEvent> {
        ...
        public void onApplicationEvent(ContextRefreshedEvent contextRefreshedEvent) {
                ...
                User secondUser =
                                this.createUser("Carlos Santos", SECOND_USER_EMAIL, UserRole.USER);

                Environment lab = this.createEnvironment("Laboratório de Informática 02",
                                "Laboratório com 30 computadores, projetor e acesso à internet.",
                                30, "Bloco B");
                Environment classroom = this.createEnvironment("Sala de Aula 101",
                                "Sala climatizada com projetor, quadro branco e 50 lugares.", 50,
                                "Bloco A");
                Environment auditorium = this.createEnvironment("Auditório Central",
                                "Auditório completo para eventos, com palco e sistema de som.", 220,
                                "Bloco A");
                Environment chemistryLab = this.createEnvironment("Laboratório de Química 03",
                                "Laboratório com bancadas, capela de exaustão e vidrarias de química.",
                                30, "Bloco C");
        ...
        private User createUser(String name, String email, UserRole role) {
                return userRepository.findUserByEmail(email).map(userDetails -> (User) userDetails)
                                .map(this::ensureTestPassword).orElseGet(() -> {
                                        User user = new User();
                                        user.setName(name);
                                        user.setEmail(email);
         ...
         *
         * <p>
         * Se o usuário foi criado antes com outra senha, o hash BCrypt é substituído por
         * {@link #DEFAULT_PASSWORD}, preservando o restante dos dados de teste.
         * </p>
         *
         ...
         *
         * <p>
         * Garante que o {@link RoleSeeder} tenha executado antes, mas não depende da ordem dos
         * listeners do contexto.
         * </p>
         *
        ...
        private Environment createEnvironment(String name, String description, Integer capacity,
                ...
                return environmentRepository.findAll().stream()
                                ...
                                .findFirst().orElseGet(() -> {
                                        ...
                                        environment.setImageUrl("https://storage.example.com/"
         ...
         *
         * <p>
         * As reservas cobrem os três status do sistema — pendentes (fila de aprovação), aprovadas
         * (ocupam horário e compõem a agenda do dia) e recusadas — e somam horas ao painel da tela
         * de início. Sempre respeitam as regras de negócio: data presente ou futura, slots de 1
         * hora em hora cheia, dentro de 08:00–22:00.
         * </p>
         *
        ...
        private void createReserves(User owner, User admin, User user, User secondUser,
                ...
                this.createReserve(secondUser, chemistryLab,
                                "Prática de laboratório da disciplina de Química Geral", 24,
                                tomorrow, LocalTime.of(14, 0), LocalTime.of(16, 0),
                ...
                this.createReserve(admin, auditorium,
                                "Palestra de abertura do congresso de extensão", 180, today,
                                LocalTime.of(8, 0), LocalTime.of(10, 0), ReserveStatus.APPROVED);
                ...
                this.createReserve(user, chemistryLab,
                                "Ensaio do grupo de pesquisa em química ambiental", 12, nextWeek,
                                LocalTime.of(15, 0), LocalTime.of(17, 0), ReserveStatus.APPROVED);

                this.createReserve(owner, auditorium, "Reunião do conselho universitário", 60,
                                nextWeek, LocalTime.of(10, 0), LocalTime.of(12, 0),
                                ReserveStatus.PENDING);
        }
        ...
        private void createReserve(User user, Environment environment, String justification,
                        Integer numberOfParticipants, LocalDate date, LocalTime startTime,
                        LocalTime endTime, ReserveStatus status) {
